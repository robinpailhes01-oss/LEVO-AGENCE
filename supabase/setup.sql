-- ============================================================================
-- LEVO — SETUP one-shot (À EXÉCUTER UNE FOIS, sur le projet Supabase Levo)
--
-- ⚠️ DESTRUCTIF : ce script SUPPRIME toutes les tables du schéma `public`
-- (y compris les tables aux noms français créées par erreur), PUIS crée le
-- schéma Levo correct attendu par le code (noms + colonnes anglais exacts).
--
-- Le projet Levo est neuf/vide → aucune donnée perdue. Ne PAS relancer ce
-- script une fois qu'il y a de vraies données (il viderait tout). Pour les
-- évolutions futures, utiliser des migrations ciblées.
-- ============================================================================

-- 0) Reset du schéma public (drop de toutes les tables existantes) -----------
do $$
declare r record;
begin
...
create index idx_content_status on public.content_calendar(status);
...
create index idx_perf_content on public.content_performance(content_id);
...
create index idx_logs_created on public.agent_logs(created_at desc);
...
create index idx_reports_week on public.weekly_reports(week_start desc);
...
-- RLS : activée partout, aucune policy anon (le serveur passe par service-role).
